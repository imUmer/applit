...
const Cards = ({ items }) => {
  ...
  return (
    <div className="cards-container colors">
      {items.map((item, index) => (
        <div key={index} className="card">
          <img src={item.image} alt={`Card ${index}`} />
          <div className="colors card-info">
            <h3>{item.title}</h3>
            <p>{item.description}</p>
            <button className="colors" onClick={() => launchFunction((item.imagename))}>{!loading ? <>Go</> : loading && <Loading text="please wait..." />}</button>
          </div>
        </div>
      ))}
    </div>
